...
parser.add_argument('--img_root', type=str,
                    default='./datasets', help='path name of image dataset')
parser.add_argument('--s_init', type=str, required=True,
                    help='initial parameters of student model')
parser.add_argument('--t_model', type=str, required=True,
                    help='path name of teacher model')
...
parser.add_argument('--t_name', type=str, required=True,
                    help='name of teacher')    # resnet20/resnet110
parser.add_argument('--s_name', type=str, required=True,
                    help='name of student')    # resnet20/resnet110
...
def main():
    ...
    snet = define_tsnet(
        name=args.s_name, num_class=args.num_class, cuda=args.cuda)
    ...
    tnet = define_tsnet(
        name=args.t_name, num_class=args.num_class, cuda=args.cuda)
...
def train(train_loader, nets, optimizer, criterions, epoch):
    ...
    for i, (img, target) in enumerate(train_loader, start=1):
        ...
        out_s = snet(img)  # logit value:not apply softmax
        out_t = tnet(img)  # logit value:not apply softmax

        # student loss with hard_targets
        cls_loss = criterionCls(out_s, target)

        # student loss with soft targets
        kd_loss = criterionKD(out_s, out_t.detach()) * \
            args.lambda_kd

        loss = cls_loss + kd_loss  # total loss
...
def test(test_loader, nets, criterions, epoch):
    ...
    logging.info(
        'Cls: {:.4f}, KD: {:.4f}, Prec@1: {:.2f}, Prec@5: {:.2f}'.format(*f_l))
